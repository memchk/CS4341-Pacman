...
//#define TBADVANCE(X) while(!Verilated::gotFinish() && tb.m_tickcount < X) { tb.tick(); }
#define TBADVANCE(X) for(int _TBADVANCE_t = 0; (!Verilated::gotFinish() && _TBADVANCE_t < X); ++_TBADVANCE_t) { tb.tick(); }
